...
// this will throw 502 bad gateway on nginx
class LimitedServerTransportProvider extends RatchetTransportProvider
{
    public $config;
    ...
    public function __construct($config)
    {
        ...
        $this->config = $config;
        $this->limit = $config['max_connections'];
    ...
    public function handleRouterStartX(RouterStartEvent $event)
    {
    ...
    public function handleRouterStart(RouterStartEvent $event)
    {
        ...
        $socket = new LimitingServer(new Reactor('tcp://' . $this->address . ':' . $this->port, $this->loop)
                    , $this->limit);

        $socket->on('error', function (\Exception $e) {
            Logger::error($this, "Got socket server error: " .$e->getMessage());
        });
